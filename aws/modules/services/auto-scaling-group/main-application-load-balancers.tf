...
# 6. aws_lb_listener - Configures the LB to 'listen' for specified requests (`http_port`)
...
###
resource "aws_lb" "lb-asg-vms" {
  name               = var.load_balancer_name
  load_balancer_type = "application"
  subnets            = data.aws_subnets.default.ids
  security_groups    = [aws_security_group.sg-asg-lb.id]
}
###
# Load Balancer Listener (lb_listener)
# Sets a 'Default' response (if no rule matches)
###
resource "aws_lb_listener" "lb-listener-http" {
  load_balancer_arn = aws_lb.lb-asg-vms.arn
  port              = var.http_port
  protocol          = "HTTP"
...
###
resource "aws_lb_target_group" "lb-targets-asg" {
...
###
# LB Listener Rule to forward HTTP `http_port` Requests on to 'Target' ASG via `server_port`
###
resource "aws_lb_listener_rule" "lb-listener-rule-asg-vms" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.lb-targets-asg.arn
  }
}